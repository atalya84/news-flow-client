...
export const AuthForm: FC<FormProps> = ({ type, onClick }) => {
	...
	const [passConfValid, setPassConfValid] = useState<FieldValidation>({
		isValid: true,
		errorText: '',
	});
	...
	const [passwordValid, setPasswordValid] = useState<FieldValidation>({
		isValid: true,
		errorText: '',
	});
			...
			try {
				...
				if (axios.isAxiosError(error)) {
					if (error.response?.data === 'User already exists') {
						...
						error.response?.data === 'User doesnot exists'
					) {
						setEmailValid({
							isValid: false,
	...
	const validateForm = () => {
		...
		if (password.length === 0) {
			...
			setPasswordValid({
				isValid: false,
				errorText: 'Password has to be at least 8 chracters',
			});
		} else {
	...
	return (
		<div className="login-form">
			...
			<TextInput
				...
				isValueValid={passwordValid.isValid}
			...
			{!isLogin && (
				<TextInput
					...
					isValueValid={passConfValid.isValid}
					errorText={passConfValid.errorText}
				/>
			)}
			...
			<Divider
				sx={{ marginY: 3, color: 'whitesmoke', borderColor: 'whitesmoke' }}
			>
				----------------------------------- Or Continue With
				-----------------------------------
			</Divider>
